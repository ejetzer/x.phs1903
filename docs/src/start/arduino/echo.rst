...
#. Ouvrez le moniteur série de l'IDE Arduino.
#. Assurez vous que le débit attendu par le moniteur série est le même
   qu'attendu par le programme Arduino. Par défaut, :cpp:class:`phs::EchoSerie`
   utilise un débit maximal de 9600.
#. Vérifiez que l'IDE Arduino envoie un caractère de fin de ligne avec
   chaque commande. À droite de la ligne de commande dans le moniteur série,
   le menu déroulant devrait indiquer :guilabel:`Nouvelle ligne`.
...
:cpp:func:`phs::LigneSerie::setup` comporte deux instructions.
...
bloc conditionnel. L'appel à :cpp:func:`phs::LigneSerie::loop` s'occupe
