...
exemples = [nom.stem for nom in dossier_courant.glob('*.py') if nom.stem != '__main__']

lecteur.add_argument('--aide', action='help', help='afficher ce message d\'aide et quitte')
lecteur.add_argument('-v', '--version', action='version',
    version=f'xphs1903 {xphs1903.__version__} > %(prog)s {__version__}',
    help='affiche la version et quitte')

lecteur.add_argument('-r', '--rouler', choices=exemples, type=str, dest='script', action='store', default=None,
    help='exécuter le script sélectionné')
lecteur.add_argument('-c', '--copier', choices=exemples, type=str, dest='source', action='append', default=[],
    help='copier le script (cumulatif)')
lecteur.add_argument('-i', '--init', action='store_true', dest='init',
    help='initialiser le projet avec un fichier README.md et requirements.txt')
lecteur.add_argument('-d', '--dest', type=Path, default=Path.cwd(), dest='dest', action='store',
    help='répertoire cible (par défaut: ".")')

lecteur.add_argument('--resp', type=str, default='Caroline Boudoux', dest='resp', action='store',
    help='responsable du cours (par défaut: %(default)r)')
lecteur.add_argument('--maitre', type=str, default='Jérémie Villeneuve', dest='maitre', action='store',
    help='maître d\'enseignement (par défaut: %(default)r)')
lecteur.add_argument('--labos', type=str, default='Guillaume Ramadier', dest='labos', action='store',
    help='chargé de laboratoire (par défaut: %(default)r)')
lecteur.add_argument('--charge', type=str, default='{charge}', dest='charge', action='store',
    help='chargé de projet (par défaut: %(default)r)')
lecteur.add_argument('--etudiant', type=str, dest='etudiants', action='append', default=[],
    help='étudiants (cumulatif) (par défaut: %(default)r)')
lecteur.add_argument('--desc', type=str, dest='desc', action='store', default='{description}',
    help='description du projet (par défaut: %(default)r)')
lecteur.add_argument('--titre', type=str, dest='titre', action='store', default='{titre}',
