...
// Posted by wildplasser, modified by community. See post 'Timeline' for change
// history Retrieved 2025-11-24, License - CC BY-SA 3.0
  ...
  for (idx_t i = 0; i < (N >> 1); i++)
    {
      ...
      float_t poids
          = 0.5
            * (1.0
               - cos (TWO_PI * (float_t)i_moins_un / (float_t)idx_moins_un));
      ...
      for (idx_t j = 0; j < L_1; j++)
        {
          for (idx_t i = j; i < N; i += L_2)
            {
              idx_t i_1 = i + L_1;
              val_t t_1 = u_1 * (float_t)(reel[j][i_1])
                          - u_2 * (float_t)(imag[j][i_1]);
              val_t t_2 = u_1 * (float_t)(imag[j][i_1])
                          + u_2 * (float_t)(reel[j][i_1]);
